...
/***oldja
    ...
    各文字は @e 文字プロパティ と呼ぶプロパティを0個以上持つことができ
    る。文字プロパティは @e キー と @e 値 からなる。キーはシンボルであ
    り、値は <tt>(void *)</tt> 型にキャストできるものなら何でもよい。 
    「文字 C の文字プロパティのうちキーが @c K であるもの」を簡単に
...
/***oldja
    ...
    は、対応する @e 標準結合クラス を表わす非負整数である。

    標準結合クラスを表わす非負整数の意味は、Unicode に定義されているもの
    と同じである。  */
...
/***oldja
    ...
    もしそのような比較に使用し得る単一の小文字が存在しない場合、このプ
    ロパティの値は 0xFFFF になる。この場合その文字は、
    #Mcomplicated_case_folding というキーの文字プロパティを持つ。  */
