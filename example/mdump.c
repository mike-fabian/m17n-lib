...
/***ja
    ...
    テキストを Netpbm 画像としてダンプする。 

    Netpbm 画像は現在のディレクトリに作られた "BASE.pbm" という名前の 
    ...
    る。FILE が 1 ページに納まらないほど大きい場合は、"BASE.01.pbm",
    "BASE.02.pbm" 等の名前のついた複数のファイルが作られる。
    ...
    されていれば、Netpbm 画像はファイルに書かれるのではなく、FILTER に 
    標準入力として渡される。 FILTER が "s" を含んでいれば、それは FILE 
    のベースネームに置き換えられる。このプログラムのデフォルトの振舞い
    と、FILTER に "cat > s.pbm" を指定した場合の振舞は同一である。
